-- PulsNet v0.7+ schema extensions
...
CREATE TABLE IF NOT EXISTS device_tenants (
...
CREATE TABLE IF NOT EXISTS "ConfigTemplates" (
  ...
  "TenantId" INT NULL REFERENCES tenants(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS management_actions (
  ...
  initiated_by_user_id INT NULL REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS management_action_devices (
  action_id BIGINT NOT NULL REFERENCES management_actions(id) ON DELETE CASCADE,
  device_id INT NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
  PRIMARY KEY (action_id, device_id)
);

CREATE TABLE IF NOT EXISTS mib_categories (
  id SERIAL PRIMARY KEY,
  name TEXT NOT NULL UNIQUE
);

CREATE TABLE IF NOT EXISTS mib_catalog (
  id SERIAL PRIMARY KEY,
  oid TEXT NOT NULL UNIQUE,
  name TEXT NOT NULL,
  category_id INT NULL REFERENCES mib_categories(id) ON DELETE SET NULL,
  description TEXT NULL
);
CREATE INDEX IF NOT EXISTS ix_mib_catalog_category ON mib_catalog(category_id);

CREATE TABLE IF NOT EXISTS mib_device_selections (
  device_id INT NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
  mib_id INT NOT NULL REFERENCES mib_catalog(id) ON DELETE CASCADE,
  PRIMARY KEY (device_id, mib_id)
);

CREATE TABLE IF NOT EXISTS mib_walk_logs (
  id BIGSERIAL PRIMARY KEY,
  device_id INT NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
