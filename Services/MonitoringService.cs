    ...
    public class MonitoringService : BackgroundService
    {
        ...
        private async Task PollDeviceAsync(AppDbContext db, SnmpClient snmp, Device device, CancellationToken ct)
        {
            try
            {
                ...
                // Poll any selected OIDs and store latest values
                var selected = await db.DeviceSelectedOids.Where(x=>x.DeviceId==device.Id).ToListAsync(ct);
                foreach (var s in selected)
                {
                    var val = await snmp.GetAsync(device.IpAddress, device.SnmpCommunity, s.Oid, device.SnmpPort);
                    if (!string.IsNullOrWhiteSpace(val))
                    {
                        db.DeviceOidSamples.Add(new DeviceOidSample{ DeviceId = device.Id, Oid = s.Oid, Label = s.Label, Value = val, Timestamp = DateTimeOffset.UtcNow });
                    }
                }
