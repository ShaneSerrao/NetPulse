...
namespace PulsNet.Web.Services.Snmp
{
    public class SnmpClient
    {
        ...
        public async Task<List<(string oid,string value)>> WalkAsync(string ipAddress, string community, string baseOidOrName, int port = 161, int timeoutMs = 2500)
        {
            ...
            var psi = new ProcessStartInfo
            {
                ...
                ArgumentList = { "-v2c", "-c", community, $"{ipAddress}:{port}", baseOidOrName, "-t", (timeoutMs/1000.0).ToString("0.0") },
                RedirectStandardOutput = true,
                RedirectStandardError = true
            };
            try
            {
                ...
                {
                    // Example: IF-MIB::ifDescr.4 = STRING: ether1
                    var sep = line.IndexOf("=");
                    if (sep <= 0) continue;
                    var left = line.Substring(0, sep).Trim();
                    var right = line.Substring(sep + 1).Trim();
